...
One-click configuration for AI coding tools: Claude Code, Codex CLI, Gemini CLI, OpenClaw, Kilo Code, Antigravity, Cline, Continue, Cursor, Factory Droid, and **GitHub Copilot** (config generator for `chatLanguageModels.json`). Features automated config apply/reset, connection profiles, and model mapping.
